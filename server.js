...
app.post('/api/info', (req, res) => {
  const { url } = req.body;
  if (!url) return res.status(400).json({ error: 'URL daal bhai!' });

  const isYT = url.includes('youtube.com') || url.includes('youtu.be');
  const isIG = url.includes('instagram.com');
  if (!isYT && !isIG) return res.status(400).json({ error: 'Sirf YouTube ya Instagram!' });

  const cmd = `"${YTDLP}" --dump-json --no-playlist "${url}"`;
...
app.post('/api/download', (req, res) => {
  ...
  const cmd = `"${YTDLP}" ${formatArg} --merge-output-format mp4 -o "${outputPath}.%(ext)s" "${url}"`;

  exec(cmd, { timeout: 120000 }, (error, stdout, stderr) => {
    if (error) {
      console.error('Download error:', stderr);
      return res.status(500).json({ error: 'Download fail!' });
    }

    const files = fs.readdirSync(tmpDir).filter(f => f.startsWith(filename));
    if (!files.length) return res.status(500).json({ error: 'File nahi mili!' });

    const filePath = path.join(tmpDir, files[0]);
    const ext = path.extname(files[0]);

    res.setHeader('Content-Disposition', `attachment; filename="vidsnap${ext}"`);
...
app.listen(PORT, () => console.log(`VidSnap port ${PORT} pe chal raha hai!`));
